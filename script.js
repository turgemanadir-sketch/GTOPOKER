...
const questionTemplates = {
    position: [
        "Imagine you're in a cash game and you have {cards}. You're in {position}. The action {action} to you. Based on {topic}, what's your best play?",
        "You're playing online and look down at {cards} in {position}. {action}. Considering {topic}, what should you do?",
        "Picture this: You have {cards} and you're sitting in {position}. {action}. Using {topic} principles, what's the optimal move?"
    ],
    scenario: [
        "Here's the situation: {scenario}. You're holding {cards}. Based on {topic}, what would be the best decision?",
        "You find yourself {scenario}. Your cards: {cards}. Applying {topic}, what's your play?",
        "Let's say {scenario}. You have {cards}. Using what you know about {topic}, what should you do?"
    ],
    flop: [
        "You raised pre-flop with {cards} and got called. The flop comes {flop}. Based on {topic}, what's your next move?",
        "After raising with {cards}, you see a flop of {flop}. Considering {topic}, how should you proceed?",
        "You've got {cards} and the flop is {flop}. {action}. Using {topic} strategy, what's optimal?"
    ],
    concept: [
        "Think about this: {situation}. Based on your understanding of {topic}, which option makes the most sense?",
        "Consider this scenario: {situation}. Applying {topic} principles, what would be correct?",
        "Here's a spot to think about: {situation}. Using {topic} knowledge, what's the right approach?"
    ]
};

// Helper function to extract cards from question text
function extractCards(text) {
    const cardPattern = /([AKQJT2-9][♠♥♦♣])/g;
    const cards = text.match(cardPattern);
    return cards ? cards.join('') : null;
}

// Helper function to extract flop from question text
function extractFlop(text) {
    const flopPattern = /(?:flop|board)(?:[:\s]+)?([AKQJT2-9][♠♥♦♣]\s*[AKQJT2-9][♠♥♦♣]\s*[AKQJT2-9][♠♥♦♣])/i;
    const match = text.match(flopPattern);
    return match ? match[1] : null;
}

// Function to transform existing questions to conversational format
function transformToConversational(question, lessonTopic) {
    // Choose appropriate template based on question content
    let template;
    let values = {};
    
    if (question.scenario && question.scenario.includes("Position:")) {
        // Position-based question
        const positionMatch = question.scenario.match(/Position: (\w+)/);
        template = questionTemplates.position[Math.floor(Math.random() * questionTemplates.position.length)];
        values = {
            position: positionMatch ? positionMatch[1] : "the button",
            cards: extractCards(question.question) || "pocket aces",
            action: "everyone folds",
            topic: lessonTopic
        };
    } else if (question.question.toLowerCase().includes("flop")) {
        // Flop-based question
        template = questionTemplates.flop[Math.floor(Math.random() * questionTemplates.flop.length)];
        values = {
            cards: extractCards(question.question) || "A♠K♦",
            flop: extractFlop(question.question) || "Q♠J♥5♦",
            action: "Your opponent checks",
            topic: lessonTopic
        };
    } else {
        // General concept question
        template = questionTemplates.concept[Math.floor(Math.random() * questionTemplates.concept.length)];
        values = {
    ...
    let conversationalQuestion = template;
    for (const [key, value] of Object.entries(values)) {
        conversationalQuestion = conversationalQuestion.replace(`{${key}}`, value);
    }
...
const lessons = [
    ...
    },
    {
        ...
        questions: 10,
        content: `
            <p><strong>ICM (Independent Chip Model)</strong> is a way to figure out what your tournament chips are really worth in actual money. Think of it like this: in cash games, every chip has the same value. But in tournaments, it's different - your chips change value based on the prize pool and how many players are left.</p>
            
            <h3>Why ICM Matters - Simple Explanation</h3>
            <p>Imagine you're in a tournament with 3 players left. The prizes are: 1st place gets $1000, 2nd gets $600, 3rd gets $400. Even if you have half the chips, you can't win half the money (that would be $1000). You can only win first place at most. This is why each extra chip becomes less valuable - there's a ceiling on what you can win.</p>
            
            <h3>Key ICM Concepts Made Simple</h3>
            <ul>
                <li><strong>Your first chips are most valuable:</strong> Going from 0 chips to 1000 chips is huge (you're in the game!). Going from 100,000 to 101,000 chips? Not as important.</li>
                <li><strong>Survival matters more near payouts:</strong> When you're close to making the money or a big pay jump, staying alive becomes more important than accumulating chips.</li>
                <li><strong>Big stacks can bully:</strong> If you have lots of chips, you can pressure smaller stacks who are trying to survive for pay jumps.</li>
                <li><strong>Medium stacks are in a tough spot:</strong> You're not desperate, but you can't bully either. You need to pick your spots carefully.</li>
            </ul>
            
            <div class="example-box">
                <h4>Real Game Example - What You'll See and Do</h4>
                <p><strong>Situation:</strong> You're playing an online tournament. The tournament lobby shows 11 players left, and 10 players get paid (make the money). You have 15 big blinds, which is a medium-sized stack. The chip leader raises from the button, and you look down at pocket Jacks in the small blind.</p>
                <p><strong>What you see on screen:</strong> The tournament info shows "11/523 players remaining" and "10 places paid - Next payout: $50". You can see everyone's stack sizes at your table. Three players have less than 5 big blinds.</p>
                <p><strong>Your thought process:</strong> "Normally, Jacks are really strong and I'd reraise or at least call. But we're on the bubble (one elimination away from everyone making money). There are 3 players with tiny stacks who will probably go all-in soon out of desperation. If I play this hand against the chip leader and lose, I get $0. If I fold and wait just a few hands, one of those short stacks will likely bust and I'll guarantee myself at least $50."</p>
                <p><strong>The ICM play:</strong> You fold the Jacks! It feels weird folding such a strong hand, but it's the smart play. Two hands later, a short stack goes all-in with Q-7 and busts. You made the money! Now that you're guaranteed a payout, you can start playing more normally again.</p>
            </div>
            
            <h3>When to Think About ICM</h3>
            <p><strong>ICM is MOST important when:</strong></p>
            <ul>
                <li><strong>On the bubble:</strong> When you're close to making the money (like 11 players left, 10 get paid)</li>
                <li><strong>At the final table:</strong> Especially when there are big pay jumps between positions</li>
                <li><strong>In satellite tournaments:</strong> Where multiple people win the exact same prize (like a ticket to a bigger tournament)</li>
            </ul>
            
            <p><strong>ICM matters LESS when:</strong></p>
            <ul>
                <li><strong>Early in tournaments:</strong> With hundreds of players left, play normally</li>
                <li><strong>You have a very short stack:</strong> With only a few big blinds, you need to take risks regardless</li>
                <li><strong>You have a huge stack:</strong> You can afford to take gambles without risking your tournament life</li>
            </ul>
            
            <h3>Simple ICM Strategy - What to Remember</h3>
            <p>Don't overthink it! Here's your basic game plan:</p>
            <ol>
                <li><strong>Near the bubble:</strong> If you have a medium stack, play much tighter than normal. Let the short stacks battle it out.</li>
                <li><strong>With a big stack:</strong> Put pressure on medium stacks who are trying to survive. They'll fold a lot.</li>
                <li><strong>With a short stack:</strong> You still need to look for good spots to double up. Don't just blind away.</li>
                <li><strong>At final tables:</strong> Before making big decisions, check the payout structure. Is there a huge jump from 4th to 3rd? Maybe wait for someone else to bust first.</li>
            </ol>
            
            <p><strong>Remember:</strong> ICM is about tournament survival and maximizing your real money equity, not just accumulating chips. Sometimes the "wrong" play in a cash game is the "right" play in a tournament!</p>
        `,
        quiz: [
            {
...
startQuizBtn.addEventListener('click', () => {
    ...
    // Set reflection text with lesson topic
    reflectionText.textContent = `Do you feel the topic "${lesson.title}" is solid in your head? Here are a few questions to check your understanding.`;
    
    reflectionPrompt.classList.remove('hidden');
    
    // Add event listener for starting quiz from reflection
    document.getElementById('startQuizFromReflection').addEventListener('click', () => {
...
function showQuizQuestion() {
    ...
    // Transform question to conversational format
    const conversationalQuestion = transformToConversational(question, lesson.title);
    
    // Show conversational question
    quizQuestion.textContent = conversationalQuestion;
    // Keep scenario for additional context if needed (but could be empty now)
    quizScenario.textContent = '';
...
function showQuizComplete() {
    ...
    if (percentage === 100) {
        ...
        if (iconEl) iconEl.textContent = '✅';
        if (titleEl) titleEl.textContent = 'Quiz Passed';
    } else {
        if (iconEl) iconEl.textContent = '📊';
        if (titleEl) titleEl.textContent = 'Keep Learning';
    }
    
    // Show appropriate buttons based on score
    if (percentage >= 60) {
        // Passed - show single continue button
        actionsEl.innerHTML = '<button class="btn btn-primary" id="continueBtn">Continue Learning</button>';
        ...
        // Failed - show retry and continue anyway options
        actionsEl.innerHTML = `
            <div class="quiz-failed-actions">
                <p class="failed-message">Score below 60% - You can retry or continue anyway</p>
                <div class="action-buttons">
                    <button class="btn btn-secondary" id="retryLessonBtn">Retry Lesson</button>
                    <button class="btn btn-primary" id="continueAnywayBtn">Continue Anyway</button>
                </div>
            </div>
        `;
        
        // Add event listeners for the new buttons
        document.getElementById('retryLessonBtn').addEventListener('click', () => {
            // Reset and restart the lesson
            lessonViewer.classList.add('hidden');
            document.body.style.overflow = 'auto';
        ...
        document.getElementById('continueAnywayBtn').addEventListener('click', () => {
            // Continue to next lesson despite low score
            lessonViewer.classList.add('hidden');
            document.body.style.overflow = 'auto';
            renderLessons();
            
            // Mark as completed anyway (for progression)
    ...
    if (percentage >= 60) {
        ...
        // Mark lesson as completed if passed threshold
        if (!completedLessons.includes(currentLessonId)) {
            completedLessons.push(currentLessonId);
            localStorage.setItem('completedLessons', JSON.stringify(completedLessons));
        }
    } else {
        message = "Don't worry! Review the lesson and try again, or continue to the next lesson when you're ready.";
    }
...
// Continue Learning - Now handled dynamically in showQuizComplete function
